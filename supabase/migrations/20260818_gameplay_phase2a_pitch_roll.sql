-- Elements Baseball Gameplay Phase 2A — pitch roll + advantage persistence
-- Run after 20260818_gameplay_phase1b_pregame_state.sql.
-- Extends the private lab writer to permit PITCH_ROLLED events.
--
...
begin
  ...
  if p_event_type not in ('PREGAME_SUBMITTED', 'PREGAME_LOCKED', 'GAME_READY', 'GAME_STARTED', 'PITCH_ROLLED', 'GAME_PAUSED', 'GAME_RESUMED') then
...
comment on function public.save_gameplay_lab_state(uuid, bigint, jsonb, text, jsonb) is
  'Private Gameplay Lab writer through Phase 2A: expected-version protected pregame, start, pitch roll, pause and resume events.';
