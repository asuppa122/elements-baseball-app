...
function rewardKind(reward:string) {
  const lower=reward.toLowerCase()
  if (lower.includes('foil')) return 'foil'
  if (lower.includes('team') || lower.includes('teammate')) return 'team'
...
function RewardGlyph({ reward }:{ reward:string }) {
  const kind=rewardKind(reward)
  return <span className={`milestone-reward-glyph reward-${kind}`} aria-hidden="true">
    {kind==='foil' ? <><i/><i/><b>✦</b></> : kind==='team' ? <><i/><span>⚑</span></> : kind==='player' ? <><i/><span>★</span></> : kind==='cards' ? <><i/><i/><span>EB</span></> : <span>◆</span>}
  </span>
}
...
export default function MilestonesPage() {
  ...
  return <main className="league-content-page milestones-page milestones-page-v3">
    <section className="milestones-hero milestones-hero-v2">
      <div className="milestones-season-copy">
        ...
        <div className="milestones-manager-row">
          ...
          <div className="milestones-live-summary"><span><strong>{tracker.totalGames}</strong>Season 10.1 GP</span><span><strong>{tracker.wins}</strong>Wins</span><span><strong>{tracker.losses}</strong>Losses</span></div>
        </div>
      </div>
      <div className="milestones-phase-progress milestones-season-timeline" aria-label="Season 10 phases">{PHASES.map((item,index)=>{ const seasonState=phaseState(item.id); return <div className={`milestones-phase-step ${seasonState}`} key={item.id}><button type="button" className={phase===item.id?'milestones-phase-card active':'milestones-phase-card'} onClick={()=>{setPhase(item.id);setTab('standard')}}><span className="phase-state-seal">{seasonState==='complete'?'✓':seasonState==='current'?'●':index+1}</span><span className="phase-label">{item.label}</span><strong>{item.weeks}</strong><em>{item.subtitle}</em><small>{seasonState==='complete'?'Completed':seasonState==='current'?'Now Playing':'On Deck'}</small></button>{index<PHASES.length-1&&<span className="milestones-phase-connector"><i /></span>}</div>})}</div>
    </section>
    ...
    <div className="milestones-layout milestones-layout-v2 milestones-layout-full"><section className={`milestones-list-panel milestones-progression-panel milestone-panel-${tab}`}><div className="league-panel-heading milestones-panel-heading"><div className="milestones-heading-lockup"><span className={`milestones-category-crest crest-${tab}`}><MilestoneGlyph type={tab} /></span><div><span>{CATEGORY_COPY[tab].eyebrow}</span><h2>{heading}</h2>{CATEGORY_COPY[tab].description && <p>{CATEGORY_COPY[tab].description}</p>}</div></div></div>
      <div className="milestone-track" role="list">{rows.map((row,index)=>{ const p=progressFor(index,row.threshold); const unavailable='unavailable' in p && p.unavailable; const complete=!unavailable && p.current>=p.target; const pct=unavailable?0:Math.min(100,p.target?Math.round((p.current/p.target)*100):0); const remaining=Math.max(0,p.target-p.current); const firstIncomplete = !unavailable && !complete && rows.slice(0,index).every((prior,priorIndex)=>{ const pp=progressFor(priorIndex,prior.threshold); return !('unavailable' in pp && pp.unavailable) && pp.current>=pp.target }); const stateClass=complete?'is-complete':firstIncomplete?'is-next':'is-upcoming'; const major=isMajorThreshold(p.target); return <article className={`milestone-achievement ${stateClass} ${major?'is-major':''}`} role="listitem" key={`${tab}-${phase}-${row.threshold}-${index}`}>
        <div className="milestone-track-rail"><span className="milestone-node">{complete?'✓':index+1}</span>{index<rows.length-1&&<span className="milestone-node-line"/>}</div>
        <div className="milestone-achievement-threshold"><div className={`milestone-plaque ${complete?'earned':firstIncomplete?'active':'locked'} ${major?'major':''}`}><span className="milestone-plaque-top">{complete?'Earned':firstIncomplete?'Chasing':'Milestone'}</span><strong>{parseThreshold(row.threshold)}</strong><span>{row.threshold.replace(/^\d+\s*/, '') || p.unit}</span>{major && <b>★</b>}</div></div>
        <div className="milestone-achievement-copy"><span className="milestone-reward-label"><RewardGlyph reward={row.reward} /> Reward</span><p>{row.reward}</p>{major && <span className="milestone-major-badge">Major Checkpoint</span>}</div>
        <div className="milestone-achievement-progress"><div className="milestone-progress-heading"><span>{unavailable?'Awaiting Workbook Tracker':complete?'Unlocked':firstIncomplete?'Next Up':'Progress'}</span><strong>{unavailable?'—':`${p.current} / ${p.target}`}</strong></div><div className="milestone-progress-track"><span style={{width:`${pct}%`}} /></div><div className="milestone-progress-meta"><span>{unavailable?'Weekly progress not connected':complete?'Milestone achieved':`${remaining} ${p.unit} remaining`}</span><em>{p.unit}</em></div></div>
      </article>})}</div>
    </section></div>
  </main>
}
