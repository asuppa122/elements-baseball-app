import { beginPrePitchDecision, confirmManagerDecision, getDecisionView, getPrePitchActions, resolveDecisionRoll, validateDecisionSelection, type RunnerAttempt } from './decisionEngine'
...
export function runNonGbScenarioMatrix(initial:GameState):ScenarioReport {
  ...
  for(const [roll,expected] of [[1,'SWING_ROLL'],[2,'SWING_ROLL'],[3,'OUT'],[4,'OUT'],[5,'OUT'],[6,'OUT'],[20,'OUT']] as const){test(`BUNT-NW-${roll}`,'Sac bunt',`No-wheel sac bunt boundary roll ${roll}`,()=>{const s=fixture(initial);s.bases.first=runner('R1','Runner');const n=resolveDecisionRoll(pending(s,'SAC_BUNT_RTS',offense(s),{}),roll);if(expected==='SWING_ROLL')assert(n.waitingFor==='SWING_ROLL','Expected return to pitcher-chart Swing.');else assert(n.outs>=1,'Expected an out on this bunt result.')})}
