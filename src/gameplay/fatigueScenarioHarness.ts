...
export function runFatigueScenarioMatrix():ScenarioReport{
 ...
 test('FAT-IP-04','IP conversion','7.33 IP equals 22 outs',()=>assert(cardIpOuts(7.33)===22,'Expected 22.'))
 test('FAT-IP-05','IP conversion','7.67 IP equals 23 outs',()=>assert(cardIpOuts(7.67)===23,'Expected 23.'))
 ...
 test('FAT-SO-01','Shutout bonus','Completing 7.33 IP immediately drops C5 to C4',()=>assert(effectivePitcherControl({printedControl:5,cardIp:7.33,outsRecorded:22,earnedRunsAllowed:0,inheritedRunnersScored:0})===4,'Expected C4 at 7.33 IP.'))
 test('FAT-SO-02','Shutout bonus','Next out after 7.33 IP drops C5 to C3',()=>assert(effectivePitcherControl({printedControl:5,cardIp:7.33,outsRecorded:23,earnedRunsAllowed:0,inheritedRunnersScored:0})===3,'Expected C3.'))
 ...
 test('FAT-SO-04','Shutout bonus','Earned run after SOB immediately costs 2, then subsequent outs cost 2 each',()=>{assert(effectivePitcherControl({printedControl:5,cardIp:7.33,outsRecorded:24,earnedRunsAllowed:1,shutoutBonusBrokenAtOuts:24})===0,'Expected C0 immediately after run after three SOB outs.');assert(effectivePitcherControl({printedControl:5,cardIp:7.33,outsRecorded:25,earnedRunsAllowed:1,shutoutBonusBrokenAtOuts:24})===-2,'Expected C-2 after next out.')})
